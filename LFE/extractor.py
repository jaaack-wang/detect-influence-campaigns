...
def get_feature_name_list():
    return list(fs.FEATURE_DICT.keys())
...
def get_text_feature_fre(raw_text, normalized_rate=100):
...
def get_text_feature_counts(raw_text):
    ...
    num_words = fs.word_count(tagged_text)
    num_chars = fs.total_char(tagged_text)
    lemmas = set(tp.get_texts_lemmas(raw_text))

    # start calculating feature frequency
    sub_data.extend([num_chars])
    ...
    return [num_words] + sub_data, lemmas
